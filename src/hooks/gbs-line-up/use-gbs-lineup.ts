...
export const useGBSLineup = (
  ...
  const [editingScheduleMemo, setEditingScheduleMemo] = useState<
    Record<string, boolean>
  >({});
  const [scheduleMemoValues, setScheduleMemoValues] = useState<
    Record<string, string>
  >({});
  ...
  const filteredData = useMemo(() => {
    ...
    if (includedSchedules.length > 0 || excludedSchedules.length > 0) {
      temp = temp.filter(row => {
        const hasAllIncludedSchedules =
          includedSchedules.length === 0 ||
          includedSchedules.every(
            scheduleKey => row.schedule[scheduleKey] === true
          );

        const hasNoExcludedSchedules =
          excludedSchedules.length === 0 ||
          excludedSchedules.every(
            scheduleKey => row.schedule[scheduleKey] === false
          );
    ...
    async (row: GBSLineupRow) => {
      ...
      try {
        ...
        if (updatedData && newGbsNumber !== null) {
          const gbsGroup = updatedData.filter(
            (r: any) => r.gbsNumber === newGbsNumber
          );
      ...
      try {
        if ((memo && memo.trim()) || color !== undefined) {
          const processedColor =
            color === "" ? null : color ? color.trim() : undefined;
