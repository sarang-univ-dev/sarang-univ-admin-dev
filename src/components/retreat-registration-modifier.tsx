...
import { TRetreatUserRegistration, ReteratRegisterUserType, RetreatRegisterStatus, TRetreatRegisterSchedule } from '@/app/types'
...
const useUserModifications = () => {
  const [modifiedUsers, setModifiedUsers] = useState<Record<number, UserModification>>({})
  const [editableRows, setEditableRows] = useState<Record<number, boolean>>({})

  const updateUserType = useCallback((userId: number, newType: ReteratRegisterUserType | undefined, originalType?: ReteratRegisterUserType) => {
    setModifiedUsers(prev => {
      if (originalType === newType) {
        const { type, ...rest } = prev[userId] || {}
        return Object.keys(rest).length ? { ...prev, [userId]: rest } : { ...prev, [userId]: {} }
      }
      return { ...prev, [userId]: { ...prev[userId], type: newType === undefined ? null : newType } }
    })
  }, [])
  ...
  const updateUserSchedule = useCallback((
    ...
    setModifiedUsers(prev => {
      const currentSchedules = prev[userId]?.retreat_register_schedule_ids || originalSchedules
      const newSchedules = checked 
        ? [...currentSchedules, scheduleId]
        : currentSchedules.filter(id => id !== scheduleId)

      const isScheduleChanged = JSON.stringify([...newSchedules].sort()) !== JSON.stringify([...originalSchedules].sort())
  ...
  const isModified = useCallback((userId: number) => {
...
const useSearch = (retreatUserRegistrations: TRetreatUserRegistration[]) => {
  ...
  const [searchResults, setSearchResults] = useState<TRetreatUserRegistration[]>([])
...
export function RetreatRegistrationModifierComponent({
  ...
  const { searchTerm, setSearchTerm, searchResults } = useSearch(retreatUserRegistrations)

  const handleUserTypeChange = (userId: number, newType: ReteratRegisterUserType | undefined) => {
    const originalUser = searchResults.find(user => user.id === userId)
    updateUserType(userId, newType, originalUser?.type)
  }

  const handleScheduleChange = (userId: number, scheduleId: number, checked: boolean) => {
    const originalUser = searchResults.find(user => user.id === userId)
  ...
  const handleSubmit = async (userId: number) => {
    ...
    if (modifications.retreat_register_schedule_ids) {
      await mockUpdateUserSchedule(userId, modifications.retreat_register_schedule_ids)
    }
  ...
  return (
    <div className="container mx-auto py-10">
            ...
            {searchResults.map((user) => (
              <TableRow key={user.id}>
                ...
                <TableCell className="text-center">
                  {getUserTypeDisplayText(modifiedUsers[user.id]?.type !== undefined ? modifiedUsers[user.id]?.type : user.type)}
                </TableCell>
                {retreatRegisterSchedules.map(schedule => (
                  <TableCell key={schedule.id} className="text-center">
                    <Checkbox
                      checked={
                        modifiedUsers[user.id]?.retreat_register_schedule_ids?.includes(schedule.id) ??
                        user.retreat_register_schedule_ids.includes(schedule.id)
                      }
                      onCheckedChange={(checked) => handleScheduleChange(user.id, schedule.id, checked as boolean)}
                  ...
                  <div className="flex items-center justify-center space-x-2">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {(modifiedUsers[user.id]?.type ?? user.type) === ReteratRegisterUserType.NEW_COMER ? (
                          <>
                            <DropdownMenuItem onClick={() => handleUserTypeChange(user.id, ReteratRegisterUserType.SOLDIER)}>
                              군지체로 배정하기
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleUserTypeChange(user.id, ReteratRegisterUserType.STAFF)}>
                              간사로 배정하기
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleUserTypeChange(user.id, undefined)}>
                              새가족 해제하기
                            </DropdownMenuItem>
                          </>
                        ) : (modifiedUsers[user.id]?.type ?? user.type) === ReteratRegisterUserType.SOLDIER ? (
                          <>
                            <DropdownMenuItem onClick={() => handleUserTypeChange(user.id, ReteratRegisterUserType.NEW_COMER)}>
                              새가족으로 배정하기
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleUserTypeChange(user.id, ReteratRegisterUserType.STAFF)}>
                              간사로 배정하기
                        ...
                        ) : (modifiedUsers[user.id]?.type ?? user.type) === ReteratRegisterUserType.STAFF ? (
                          <>
                            <DropdownMenuItem onClick={() => handleUserTypeChange(user.id, ReteratRegisterUserType.NEW_COMER)}>
                              새가족으로 배정하기
                            </DropdownMenuItem>
                        ...
                        <DropdownMenuItem onClick={() => toggleEditing(user.id)}>
                ...
                <TableCell className="text-center">{user.price.toLocaleString()}원</TableCell>
                {Object.keys(modifiedUsers).length > 0 && (
                  <TableCell className={`text-center ${isModified(user.id) ? 'text-blue-600 font-bold' : 'text-gray-400'}`}>
                    {isModified(user.id) ? 
      ...
      {searchTerm.trim() !== '' && searchResults.length === 0 && (
        <p className="mt-4 text-center">"{searchTerm}"에 대한 검색 결과가 없습니다.</p>
      )}
    </div>
  )
}

const mockUpdateUserType = async (userId: number, newType: ReteratRegisterUserType | undefined) => {
  await new Promise(resolve => setTimeout(resolve, 500))
  console.log(`Updated user ${userId} to type ${newType}`)
}

const mockUpdateUserSchedule = async (userId: number, newSchedule: number[]) => {
  await new Promise(resolve => setTimeout(resolve, 500))
